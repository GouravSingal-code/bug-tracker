  ...
  .then(data=>{
    ...
    for( let i=0 ; i<data.project.length ; i++ ){
      for( let j=0  ; j<data.project[i].users.length ; j++ ){
        if( data.project[i].users[j].email == email ){
          let op1 = document.createElement('option');
          op1.innerHTML+=`${data.project[i].title}`;
          let op2 = document.createElement('option');
          op2.innerHTML+=`${data.project[i].title}`;
          let op3 = document.createElement('option');
          op3.innerHTML+=`${data.project[i].title}`;
          document.getElementById("project").appendChild(op1);
          document.getElementById("project2").appendChild(op2);
          document.getElementById("project3").appendChild(op3);
        }
      }
    }
...
function set(project){
  ...
  var a = document.getElementById("add_follower1");
  while(a.options.length > 0) {
    a.remove(0);
  }

  var b = document.getElementById("assign_to1");
  while(b.options.length > 0) {
    b.remove(0);
  }

  var c = document.getElementById("user2");
  while(c.options.length > 0) {
    c.remove(0);
  }

  var d = document.getElementById("user3");
